...
class CatalogosDistancia extends CI_Controller {
	...
	function index()
	{
	    ...
	    $crud->set_table('rist_agenda')
	        ...
	        ->columns('a_nombre','a_registro','a_registro_fin','a_inicio','a_hr_inicio','a_fin','a_hr_fin','a_duracion','a_desc','a_liga','a_estado')
	    ...
	    $crud->fields('a_nombre','a_registro','a_tipo','a_registro_fin','a_inicio','a_hr_inicio','a_fin','a_hr_fin','a_duracion','a_desc','a_liga','a_estado');
	    $crud->required_fields('a_nombre','a_registro','a_registro_fin','a_inicio','a_hr_inicio','a_fin','a_hr_fin','a_estado');

	    
        $crud->set_rules('a_registro', 'Fecha de inicio de registro', 'callback_fechas_registros['.$this->input->post('a_registro_fin').']');
        //$crud->set_rules('a_registro_fin', 'Fecha de fin de registro', 'callback_fechas_registro['.$this->input->post('a_inicio').']');
    ...
    public function correoactualizacion($sesion_id){
        ...
        if(isset($students) && !empty($students)){ 		
			foreach($students as $id=>$student){
				...
				$mail->Subject = 'Notificación actualización de información  :: Talleres IMSS';
